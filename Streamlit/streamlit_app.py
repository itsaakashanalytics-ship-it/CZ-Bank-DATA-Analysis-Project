...
#
# On top of that, simple single-line "Label : value" stats (e.g.
# "Total Clients     : 5,369") are detected and rendered as st.metric()
# cards — grouped 4-per-row — instead of plain text, so KPI lines look
# like the big-number dashboard cards rather than a console dump.
# Section headers ("===== A1. CUSTOMER DEMOGRAPHICS =====") become
# st.subheader(). Anything else (multi-line tables, classification
# reports, warnings) still renders as plain monospace text.
_HEADER_RE = re.compile(r'^=+\s*(.*?)\s*=+$')
_STAT_RE   = re.compile(r'^(.+?)\s*:\s*(.+)$')


def _looks_like_metric_value(value):
    """True only for short, single-stat values — not lists, dicts, or
    compound multi-value strings — so we don't turn something like
    "Dataset: 616 rows | Features: [...]" into a misleading metric card."""
...
def _run_capturing_output(func, *args, **kwargs):
    ...
    def _flush_metrics():
        if not pending_metrics:
            return
        for i in range(0, len(pending_metrics), 4):
            chunk = pending_metrics[i:i + 4]
    ...
    def _live_print(*args_, **kwargs_):
        ...
        if len(lines) == 1 and lines[0].strip():
            ...
            if header_match:
                _flush_metrics()
                st.subheader(header_match.group(1))
                return

            stat_match = _STAT_RE.match(line)
            if stat_match:
                label, value = stat_match.group(1).strip(), stat_match.group(2).strip()
                if _looks_like_metric_value(value):
                    pending_metrics.append((label, value))
                    return

        # Anything else (multi-line tables, reports, blank lines, non-stat
        # single lines) — flush any pending metric row first to keep order.
        _flush_metrics()
        if stripped:
            st.text(stripped)  # renders immediately, in document order
    ...
    _flush_metrics()  # render any trailing stats that had no line after them
...
with tab_try:
    ...
    if st.session_state.pred_results is None:
        ...
        info = results['c1']
        st.subheader("C1 — Loan Default Risk")
