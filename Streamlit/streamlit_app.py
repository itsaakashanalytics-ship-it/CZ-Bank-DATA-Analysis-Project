...
# ── Helper: run a function while capturing its print() output ───────
...
source = st.sidebar.radio("Data source", ["CSV files", "SQL Server"], index=0)
...
if source == "CSV files":
    data_dir = st.sidebar.text_input("Data folder (raw CSVs)", value="./data")
    sep = st.sidebar.selectbox("CSV separator", [";", ","], index=0)

    if st.sidebar.button("📥 Load & Prepare Data", type="primary"):
        try:
    ...
    server = st.sidebar.text_input("Server", value=_secret("sql_server"),
                                    placeholder="e.g. MYSERVER\\SQLEXPRESS or myserver.database.windows.net")
    database = st.sidebar.text_input("Database", value=_secret("sql_database"))
    port = st.sidebar.number_input("Port", value=int(_secret("sql_port", 1433) or 1433), step=1)
    schema = st.sidebar.text_input("Schema (optional)", value=_secret("sql_schema", "dbo"),
                                    help="Leave blank if your tables aren't schema-qualified.")

    if dialect_value == 'pyodbc':
        auth_mode = st.sidebar.radio("Authentication", ["Windows / Trusted", "SQL login"], index=0)
        trusted = auth_mode == "Windows / Trusted"
    else:
        st.sidebar.caption("pymssql requires a SQL Server login (Windows/Trusted auth isn't supported).")
        trusted = False

    username = password = ""
    if not trusted:
        username = st.sidebar.text_input("Username", value=_secret("sql_username"))
        password = st.sidebar.text_input("Password", value=_secret("sql_password"), type="password")
    driver = st.sidebar.text_input("ODBC driver", value="ODBC Driver 17 for SQL Server",
                                    disabled=(dialect_value == 'pymssql'))

    with st.sidebar.expander("Override table names (optional)"):
        st.caption("Only fill in tables whose name differs from the default (lowercase table name).")
        table_overrides = {}
    ...
    if st.sidebar.button("📥 Load & Prepare Data", type="primary"):
        if not server or not database:
            st.sidebar.error("Please enter both Server and Database.")
        else:
            try:
                ...
                st.sidebar.success("Data loaded successfully from SQL Server.")
            except Exception as e:
                st.sidebar.error(f"Failed to load data: {e}")
...
if st.session_state.data is None:
    st.info("👈 Set your data folder in the sidebar and click **Load & Prepare Data** to begin.")
    st.stop()
