...
#   C3. Card Adoption Prediction  (Logistic Regression)
#
...
def run_predictive_analysis(data, model_dir='model', show=True):
    """
    Runs Section C (C1-C3): trains, evaluates, plots, and pickles all 3 models.
    ...
    RocCurveDisplay.from_predictions(y1_te, y1_prob, ax=axes[1], color=NAVY)
    ...
    # ── C3. CARD ADOPTION PREDICTION (Logistic Regression) ───────────
    print("\n===== C3. CARD ADOPTION PREDICTION =====")
    print("Business Value: Increase card penetration")

    C3 = acc_cli.copy() if 'has_card' in acc_cli.columns else \
         account.copy().merge(card_flag[['account_id', 'has_card']], on='account_id', how='left')
    C3['has_card'] = C3['has_card'].fillna(0).astype(int)
    C3 = C3.merge(txn_feat, on='account_id', how='left')
    C3 = C3.merge(district[['district_id', 'avg_salary', 'unemp_96']], on='district_id', how='left')
    if 'is_female' not in C3.columns:
        C3['is_female'] = (C3['gender'] == 'F').astype(int) if 'gender' in C3.columns else 0
    for c in ['age', 'avg_salary', 'avg_txn_amount', 'avg_balance', 'txn_count', 'unemp_96']:
        if c in C3.columns:
            C3[c] = pd.to_numeric(C3[c], errors='coerce').fillna(C3[c].median() if c in C3.columns else 0)

    FEAT_C3 = [f for f in ['age', 'is_female', 'avg_salary', 'unemp_96',
                           'avg_txn_amount', 'avg_balance', 'txn_count'] if f in C3.columns]
    C3 = C3.dropna(subset=FEAT_C3 + ['has_card'])
    X3 = C3[FEAT_C3]
    y3 = C3['has_card']

    print(f"Dataset: {len(X3)} rows  |  Card adoption: {y3.mean()*100:.1f}%")
    _require_rows(X3, "C3 card adoption dataset",
                  "account/district/txn_feat joins produced no matching rows, or "
                  "'age'/'avg_salary'/'avg_txn_amount' etc. are missing/empty.")
    ...
    m3 = LogisticRegression(class_weight='balanced', max_iter=500, random_state=42)
    m3.fit(X3_tr_s, y3_tr)
    y3_pred = m3.predict(X3_te_s)
    y3_prob = m3.predict_proba(X3_te_s)[:, 1]
    ...
    print("\nClassification Report:")
    print(classification_report(y3_te, y3_pred, target_names=['No Card', 'Has Card']))

    fig, axes = plt.subplots(1, 3, figsize=(17, 5))
    fig.suptitle(f'C3 — Card Adoption Prediction  |  Accuracy={acc3*100:.1f}%  AUC={auc3:.3f}',
                 fontsize=13, fontweight='bold')

    ConfusionMatrixDisplay(confusion_matrix(y3_te, y3_pred),
        display_labels=['No Card', 'Has Card']).plot(ax=axes[0], cmap='Blues')
    axes[0].set_title('Confusion Matrix')

    RocCurveDisplay.from_predictions(y3_te, y3_prob, ax=axes[1], color=TEAL)
    axes[1].plot([0, 1], [0, 1], '--', color='gray', lw=1)
    axes[1].set_title(f'ROC Curve  (AUC={auc3:.3f})')

    coef3 = pd.DataFrame({'Feature': FEAT_C3, 'Coef': m3.coef_[0]}
                        ).sort_values('Coef', key=abs, ascending=True)
    axes[2].barh(coef3['Feature'], coef3['Coef'],
                 color=[GREEN if c > 0 else RED for c in coef3['Coef']])
    axes[2].axvline(0, color='black', lw=0.8)
    axes[2].set_title('Feature Importance\n(Green = more likely to adopt card)')
    add_bar_labels(axes[2], fmt='{:.2f}')
    plt.tight_layout()
    if show: plt.show()

    with open(f'{model_dir}/card_adoption_model.pkl', 'wb') as f: pickle.dump(m3, f)
    with open(f'{model_dir}/scaler_c3.pkl', 'wb') as f:           pickle.dump(sc3, f)
    with open(f'{model_dir}/features_c3.pkl', 'wb') as f:         pickle.dump(FEAT_C3, f)
    print(f"Model saved → {model_dir}/card_adoption_model.pkl")

    results['c3'] = {'accuracy': acc3, 'auc': auc3, 'model': m3, 'scaler': sc3, 'features': FEAT_C3}
